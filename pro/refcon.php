...
$dob=$_POST['dob'];
if(isset($_POST['yr']))$year = $_POST['yr'];
//$month= $_POST['month'];
//$date= $_POST['date'];
//$doe = $year."-".$month."-".$date;
$zone = $_POST['zone'];
...
if(empty($fname) or empty($lname) or empty($gender) or empty($dob) or empty($origin) or empty($zone)){
echo 'please make sure that you fill all the details';
echo "<form action=refReg.php>";
echo "<input type=submit value=ok>";
echo "</form>";
exit;
 } 

//inserting refugee data into the table called refugees
$query = "insert into refugees values ('".$id."','".$fname."','".$lname."','".$fullname."','".$gender."','".$dob."','".$zone."','".$origin."')";
$result=$conn->query($query);

//checking to see if the data has been sent or not
if(!$result){
echo "sorry,there seems a problem".mysql_error();
echo "<form action=refReg.php method=post>";
echo "<input type=submit value=ok>";
echo "</form>";
exit;
}
else{
echo "New refugee has been registered succesfully";
echo "<form action=refReg.php method=post>";
echo "<input type=submit value=ok>";
echo "</form>";
}
